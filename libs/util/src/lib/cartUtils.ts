...
export const sumTotalCost = (cart?: Cart): number => {
  if (cart && cart.cartLines) {
    return cart.cartLines.reduce((sum, cl) => {
      if (
        cl &&
        typeof cl.price === 'number' &&
        typeof cl.quantity == 'number'
      ) {
...
export const getRemainingAmount = (line: CartLine): number => {
  return roundToTwo(
    getLineTotalWithTax(line.price, line.quantity, 0) -
      getTotalAmounts(line.cartLineAccounts)
  );
};

export const getLineTotalWithTax = (
  linePrice: number,
  lineQuantity: number,
  lineTax: number
): number => {
...
export const getRemainingPercentage = (
  line: CartLine,
  excludeCartLineAccountId?: string
): number => {
...
export const getTotalPercentages = (
  line: CartLine,
  excludeCartLineAccountId?: string
): number => {
...
export const computePercentageGivenAmount = (
  account: CartLineAccount,
  line: CartLine
) => {
...
export const computeAmountGivenPercentage = (input: {
  ...
  const ltwt: number = getLineTotalWithTax(
    input.linePrice,
    input.lineQuantity,
    input.lineTax
  );
...
export const hasNegativeAmounts = (line: CartLine): boolean => {
  ...
  if (
    line?.cartLineAccounts?.length &&
    line.cartLineAccounts.some((account) => account.amount < 0)
  ) {
    return true;
  }
  return false;
